 ...
 * 
 * => F. Ivan and Burgers ( @c 2500 )
 * 
 * -> https://codeforces.com/contest/1100/problem/F
 * 
 * Submissions ~> https://codeforces.com/contest/1100/submission/353721101 By huanhuanonly
 * 
 * @b XOR-basis and @b Offline
 * 
 * ----December 16, 2025 [22h:42m:59s]----
*/
...
int main(void)
{
    ...
    using XorBasis = XorBasis<uint32, uint32, 20>;
    ...
    std::ranges::sort(queries, [](const question& lhs, const question& rhs) -> bool
    {
        if (lhs.l != rhs.l)
        {
            return lhs.l > rhs.l;
    ...
    auto move_to = [&, l = n + 1](uint32 to) mutable -> void
    {
        while (l > to)
        {
            --l;
    ...
    for (uint32 i = 0; i < q; )
    {
        ...
        for (auto it = ids.begin(); j < q and queries[i].l == queries[j].l; ++j)
        {
            while (it != ids.end() and queries[j].r >= *it)
            {
                cbasis.insert(v[*it]);
                ++it;
            }

            ans[queries[j].id] = cbasis.find_max_xor_sum();
        }

        i = j;
    }

    for (const uint32& i : ans)
    {
        std::cout << i << '\n';
    }
