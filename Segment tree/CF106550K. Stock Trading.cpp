 ...
 * => K. Stock Trading
 *
 * -> https://codeforces.com/gym/106550/problem/K
 *
 * Submissions ~> https://codeforces.com/gym/106550/submission/376518530 By huanhuanonly
 *
 * @b Prefix-sum
 * @b Binary-search
 * @b Segment-tree
...
int main(void)
{
    ...
    while (_--)
    {
        ...
        SegmentTree<
            ...
            {
                return std::pair(lhs.first + rhs.first, std::ranges::min({lhs.second, lhs.first + rhs.second}));
            })> seg(n);

        static auto add = [](auto& ref) -> void
        {
            ++ref.first;
            ++ref.second;
        };

        static auto subtract = [](auto& ref) -> void
        {
            --ref.first;
            --ref.second;
        };

        std::map<int64, std::vector<int>, std::ranges::greater> pos;

        std::pair<int64, int64> init_cnt_sum;
        ...
        std::map<int64, int64> ans;

        int64 sum = init_cnt_sum.second;

        for (const auto& [key, ids] : pos)
        {
            for (int cpos : ids)
            {
                if (seg.reduce(0, cpos).first)
                {
                    sum += v[cpos];
                    seg.apply(cpos, subtract);
            ...
            ans[key] = sum + seg.reduce().first * v.back();
        }

        int m;
        std::cin >> m;

        for (const auto max = pos.begin()->first; m--; )
        {
            int b;
            std::cin >> b;
            ...
            if (b <= max)
            {
                std::cout << ans.lower_bound(b)->second << " \n"[m == 0];
            }
            else
            {
                std::cout << init_cnt_sum.second + init_cnt_sum.first * v.back() << " \n"[m == 0];
            }
        }
    }
