 ...
 * => P6021 洪水 ( @c 省选/NOI- )
 *
 * -> https://www.luogu.com.cn/problem/P6021
 *
 * Submissions ~> https://www.luogu.com.cn/record/276962084 By huanhuanonly
 *
...
int main(void)
{
    ...
    auto dfs = [&](const auto& self, int u) -> void
    {
        ...
        segs[tree[u].head].set(pos[u], matrix22{{g[u], inf}, {w[u], 0}});
    };
    ...
    while (q--)
    {
        ...
        if (opt == 'Q')
        {
            ...
            const auto m = segs[tree[u].head].reduce(pos[u], cnt[tree[u].head] - 1);
            std::cout << std::min(m.at(0, 0), m.at(1, 0)) << '\n';
        }
        else
        {
            int u;
            ...
            {
                const auto oldm = segs[tree[u].head].reduce();

                segs[tree[u].head].set(pos[u], matrix22{{g[u], inf}, {w[u], 0}});

                const auto newm = segs[tree[u].head].reduce();

                if (const int fa = tree[tree[u].head].fa)
                {
                    g[fa] += std::min(newm.at(0, 0), newm.at(1, 0)) - std::min(oldm.at(0, 0), oldm.at(1, 0));
                }
            }
        }
    }
