 ...
 *
 * Submissions ~> https://codeforces.com/contest/25/submission/351689989 By huanhuanonly
 *
 * @b Polynomial-rolling-hash
 *
 * ----December 03, 2025 [15h:21m:47s]----
*/
...
constexpr hash_result_type hash_base = 449;
constexpr hash_result_type hash_mod  = 998244353;
...
int main(void)
{
    ...
    do
    {
        const RangeHash* last = &h[0];

        int cur = h[0].size();

        for (int i = 1; i < h.size(); ++i)
        {
            if (not last->contains(h[i]))
            {
                cur += h[i].size() - last->longest_suffix_prefix_overlap(h[i]);
                last = &h[i];
            }
        }
    ...
    } while (std::ranges::next_permutation(h).found);
