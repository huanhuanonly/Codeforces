 ...
 *
 * -> https://qoj.ac/contest/1784/problem/9249/statement/en
 *
 * Submissions ~> https://qoj.ac/submission/2345063 By huanhuanonly
 *
 * @b Segment-tree
 * @b Divide-and-conquer
 *
 * ----May 9, 2026 [11h:56m:50s]----
*/
...
int main(void)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        seg.recurse([&](const auto& info, const auto& value) -> void
        {
            if (info.is_leaf())
            {
                return;
            ...
            const auto mid = info.l + info.r >> 1;

            if (pos[i] <= mid)
            {
                info.to_left();

                if ((rem -= info.right_value()) >= 0 and info.size() <= i + 1)
                {
                    ++ans[pos[i]];
                }
            }
            else
            {
                ...
                if ((rem -= info.left_value()) >= 0 and info.size() <= i + 1)
                {
                    ++ans[pos[i]];
                }
            }
