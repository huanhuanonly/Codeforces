 ...
 * 
 * => F. Mahmoud and Ehab and yet another xor task ( @c 2400 )
 * 
 * -> https://codeforces.com/contest/959/problem/F
 * 
 * Submissions ~> https://codeforces.com/contest/959/submission/353789088 By huanhuanonly
 * 
 * @b XOR-basis
 * @b Offline
 * @b Combinatorics
 * 
 * ----December 17, 2025 [14h:30m:41s]----
*/
...
int main(void)
{
    ...
    std::vector<std::tuple<uint32, uint32, uint32>> queries(q);
    ...
    XorBasis<uint32> basis{0U};

    for (uint32 l = 1, cpos = 0; l <= n and cpos < q; ++l)
    {
        basis.insert(v[l]);

        for (; cpos < q and std::get<0>(queries[cpos]) == l; ++cpos)
        {
            if (basis.contains(std::get<1>(queries[cpos])))
            {
                ans[std::get<2>(queries[cpos])] = binary_exponentiation<uint64>(2, std::get<0>(queries[cpos]) - basis.size(), mod);
            }
        }
    }

    for (const uint64& i : ans)
    {
        std::cout << i << '\n';
    }
