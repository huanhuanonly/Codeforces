 ...
 * 
 * Submissions ~> https://atcoder.jp/contests/dp/submissions/75129655 By huanhuanonly
 * 
 * @b DP with @b Segment-tree
 * 
 * ----April 20, 2026 [17h:38m:37s]----
*/
...
int main(void)
{
    ...
    std::vector<std::vector<std::pair<int, int>>> s(n + 1), t(n + 1);
    LazySegmentTree<int64, decltype(std::ranges::max), int64, MakeLazyFunc::Add> seg(n + 1);
    ...
    for (int i = 0; i < m; ++i)
    {
        ...
        s[l].emplace_back(r, cost);
        t[r].emplace_back(l, cost);

        seg.range_update(l, r, cost);
    }

    std::vector<int64> v(n + 1);
    seg.flatten(0, seg.size() - 1, v.begin());
    
    for (int i = 1; i <= n; ++i)
    {
        for (const auto& [r, cost] : s[i])
        {
            seg.range_update(i, r, -cost);
        }
        
        seg.set(i, seg.reduce(0, i - 1));

        for (const auto& [l, cost] : t[i])
        {
            seg.range_update(l, i, cost);
        }
    }
