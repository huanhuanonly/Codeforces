 ...
 * 
 * => E. XOR on Segment ( @c 2000 )
 * 
 * -> https://codeforces.com/contest/242/problem/E
 * 
 * Submissions ~> https://codeforces.com/contest/242/submission/351881522 By huanhuanonly
 * 
 * @b Segment-tree and @b Bitwise-XOR
 * 
 * ----December 04, 2025 [22h:53m:44s]----
*/
...
int main(void)
{
    ...
    constexpr auto bitcnt = uint32(20U);
    
    uint32 n;
    std::cin >> n;

    std::array<LazySegmentTree<uint32, std::plus<uint32>, uint8, decltype(
        [](const auto& that, uint8 value) -> void {
            if (value)
            {
                that.lazy = not that.lazy;
                that.value = that.size - that.value;
            }
        })>, bitcnt> segs;
    ...
    for (uint32 i = 0; i < bitcnt; ++i)
    {
        ...
        segs[i].for_each([&, j = uint32{}](uint32& value) mutable -> bool {
            value = !!(v[j] & (1U << i));
            return ++j;
        });
    }
    ...
    while (q--)
    {
        ...
        if (opt == 1)
        {
            ...
            for (uint32 i = 0; i < bitcnt; ++i)
            {
                sum += uint64(1U << i) * segs[i].reduce(l, r);
            }
