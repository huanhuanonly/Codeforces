 ...
 * 
 * => C. Gerald and Giant Chess ( @c 2200 )
 * 
 * -> https://codeforces.com/contest/559/problem/C
 * 
 * Submissions ~> https://codeforces.com/contest/559/submission/369999705 By huanhuanonly
 * 
 * @b Counting-DP
 * @b Combinatorics
 * 
 * ----April 06, 2026 [23h:28m:50s]----
*/
...
int main(void)
{
    ...
    for (int i = 0; i < n + 1; ++i)
    {
        dp[i] = calc(v[i].first, v[i].second);

        for (int j = 0; j < i; ++j)
        {
            dp[i] += mod - dp[j] * calc(v[i].first - v[j].first + 1, v[i].second - v[j].second + 1) % mod;
            dp[i] %= mod;
        }
    }
