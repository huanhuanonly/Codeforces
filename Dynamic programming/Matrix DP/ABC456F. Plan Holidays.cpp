 ...
 *
 * => F - Plan Holidays ( @c 525 )
 *
 * -> https://atcoder.jp/contests/abc456/tasks/abc456_f
 *
 * Submissions ~> https://atcoder.jp/contests/abc456/submissions/75504766 By huanhuanonly
 *
 * @b Matrix-DP ( @a semiring-matrix-multiplication @a (min,plus) )
 * @b Dynamic-DP
 * @b Segment-tree
 *
 * ----May 04, 2026 [16h:42m:31s]----
*/
...
int main(void)
{
    ...
    while (_--)
    {
        ...
        using matrix22 = Matrix<uint64, 2, 2, init_val>;

        auto mul = [](const auto& lhs, const auto& rhs)
        {
            return lhs.semiring_multiply(rhs, std::ranges::min, std::plus{});
        };

        SegmentTree<matrix22, decltype(mul)> seg(v, [](uint64 x) -> matrix22 { return {{inf, x}, {0ULL, x}}; });
        ...
        {
            {
                ans = std::min(ans, mul(Matrix<uint64, 1, 2, init_val>{{inf, v[i]}}, seg.reduce(i + 1, i + k - 1)).at(0, 1));
            }

            if (i + k < n)
            {
                ans = std::min(ans, mul(Matrix<uint64, 1, 2, init_val>{{inf, v[i]}}, seg.reduce(i + 1, i + k)).at(0, 1));
            }
        }
