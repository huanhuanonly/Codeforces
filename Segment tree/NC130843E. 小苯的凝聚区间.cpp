 ...
 * => E-小苯的凝聚区间 ( @c 1600 )
 *
 * -> https://ac.nowcoder.com/acm/contest/130843/E
 *
 * Submissions ~> https://ac.nowcoder.com/acm/contest/view-submission?submissionId=83060005 By huanhuanonly
 *
 * @b Segment-tree
 *
 * ----March 29, 2026 [21h:09m:21s]----
*/
...
int main(void)
{
    ...
    while (_--)
    {
        ...
        LazySegmentTree<
            std::ranges::min_max_result<int>,
            decltype([](const auto& lhs, const auto& rhs) {
                return std::ranges::min_max_result{std::min(lhs.min, rhs.min), std::max(lhs.max, rhs.max)};
            }),
            int,
            decltype([](const auto& that, const auto& value) -> void {
                that.value.min += value;
                that.value.max += value;

                that.lazy += value;
            })> seg_add(n), seg_sub(n);

        seg_add.build(std::views::iota(0, n), [&](int i) { return std::ranges::min_max_result{v[i] + i, v[i] + i}; });
        seg_sub.build(std::views::iota(0, n), [&](int i) { return std::ranges::min_max_result{v[i] - i, v[i] - i}; });
        ...
        for (int i = 0; i < n; ++i)
        {
            if (i)
            {
                seg_add.range_update(i, n - 1, -1);
                seg_add.range_update(0, i - 1, 1);

                seg_sub.range_update(i, n - 1, 1);
                seg_sub.range_update(0, i - 1, -1);
            }

            ans = std::max(ans, v[i] - std::min(
                i ? seg_add.reduce(0, i - 1).min : inf,
                i + 1 < n ? seg_add.reduce(i + 1, n - 1).min : inf));

            ans = std::max(ans, -v[i] + std::max(
                i ? seg_sub.reduce(0, i - 1).max : -inf,
                i + 1 < n ? seg_sub.reduce(i + 1, n - 1).max : -inf));
        }
