 ...
 * 
 * => E1. Submedians (Easy Version) ( @c 1900 )
 * 
 * -> https://codeforces.com/contest/2128/problem/E1
 * 
 * Submissions ~> https://codeforces.com/contest/2128/submission/332076462 By huanhuanonly
 * 
 * @b Binary-search and @b Prefix-sum
 * 
 * ----August 02, 2025 [18h:22m:57s]----
*/
...
int main(void)
{
    ...
    while (_--)
    {
        ...
        auto check = [&](int mid) -> std::optional<std::pair<int, int>>
        {
            auto $ = [&](int x) -> int
            {
                return x < mid ? -1 : 1;
            ...
            for (int i = k; i <= n; ++i)
            {
                ...
                {
                    return std::make_pair(premin[i - k].second + 1, sufmax[i].second);
                }
            }
        ...
        auto ans = check(l);
        std::cout << l << ' ' << ans->first << ' ' << ans->second << '\n';
    }
