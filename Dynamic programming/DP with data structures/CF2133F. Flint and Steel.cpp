 ...
 * 
 * => F. Flint and Steel ( @c 3000 )
 * 
 * -> https://codeforces.com/contest/2133/problem/F
 * 
 * Submissions ~> https://codeforces.com/contest/2133/submission/336086461 By huanhuanonly
 * 
 * @b DP with @b Lazy-segment-tree
 * 
 * ----August 29, 2025 [13h:37m:13s]----
*/
...
public:
    ...
    constexpr void for_each(int __l, int __r, _Callback __callback)
    { _M_for_each(1, 1, size(), __l + 1, __r + 1, __callback); }
    
    template<typename _Callback>
    constexpr void for_each(_Callback __callback)
    { for_each(0, size() - 1, __callback); }
    ...
    template<typename _Tp = value_type>
    constexpr void set(int __p, const _Tp& __value = _Tp{})
    { set(__p, __p, __value); }
    ...
    template<typename _Tp = value_type>
    constexpr void set(int __l, int __r, const _Tp& __value = _Tp{})
...
protected:
    ...
    template<typename _Callback>
    constexpr bool _M_for_each(int p, int l, int r, int x, int y, const _Callback& __callback)
    {
        if (l == r)
        {
            return __callback(tree[p]);
        }
        ...
        if (x <= mid)
        {
            continues = _M_for_each(ls(p), l, mid, x, y, __callback);
        }

        if (y > mid and continues)
        {
            continues = _M_for_each(rs(p), mid + 1, r, x, y, __callback);
        }
...
struct MakeLazyFunc
{
    ...
    struct Merge
    {
        constexpr void operator()(const auto& that, const auto& value) noexcept
        {
            that.tree = that.merge(that.tree, value);
...
int main(void)
{
    ...
    while (_--)
    {
        ...
        auto merge_nodes = [](const node& lv, const node& rv) -> node
        {
            return std::min(lv, rv);
        };

        SegmentTree seg1(n + 1, node{0, inf}, merge_nodes);
        seg1.set(0, node{0, 0});
        
        LazySegmentTree seg2(n + 1, node{0, inf}, merge_nodes, MakeLazyFunc::Merge{});
        seg2.set(0, node{0, 0});

        for (const auto& i : v)
        {
            auto res = seg1.reduce(i.l - 1, i.i - 1);
            ...
            seg1.set(i.r, std::min(seg1.at(i.r), node{i.i, dp[i.i]}));
            seg2.range_update(i.i, i.r, {i.i, dp[i.i]});
        }
        ...
        for (int i = int(v.size()) - 1; i >= 0 and v[i].r == n; --i)
        {
            if (dp[v[i].i] < min)
            {
                min = dp[v[i].i];
        ...
        std::ranges::sort(ans, {}, [&e](int i) -> int { return e[i]; });
