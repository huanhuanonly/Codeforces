 ...
 *
 * => E - Crossing Table Cloth ( @c 475 )
 *
 * -> https://atcoder.jp/contests/abc457/tasks/abc457_e
 *
 * Submissions ~> https://atcoder.jp/contests/abc457/submissions/75711418 By huanhuanonly
 *
 * @b Segment-tree and @b Binary-search
 *
 * ----May 11, 2026 [00h:48m:15s]----
*/
...
int main(void)
{
    ...
    SegmentTree<int, decltype(std::ranges::min)> seg(n + 1);
    seg.fill(std::numeric_limits<int>::max());
    ...
    for (int i = 0; i < m; ++i)
    {
        ...
        seg.set(l, std::min(seg.at(l), r));
        ++cnt[{l, r}];
    }

    for (int i = 1; i <= n; ++i)
    {
        std::ranges::sort(ls[i], std::ranges::greater{});
        std::ranges::sort(rs[i]);
    }
    ...
    while (q--)
    {
        ...
        auto lit = std::ranges::lower_bound(ls[l], r, std::ranges::greater{});
        auto rit = std::ranges::lower_bound(rs[r], l);

        if (lit != ls[l].end() and rit != rs[r].end() and *lit + 1 >= *rit and (cnt[{l, r}] >= 2 or seg.reduce(l, r) < r or std::next(rit) != rs[r].end()))
        {
            std::cout << "Yes\n";
        }
        else
