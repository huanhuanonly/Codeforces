 ...
 * 
 * => B. Optimal Partition ( @c 2100 )
 * 
 * -> https://codeforces.com/contest/1667/problem/B
 * 
 * Submissions ~> https://codeforces.com/contest/1667/submission/339914920 By huanhuanonly
 * 
 * @b DP with @b Segment-tree
 * 
 * ----September 22, 2025 [23h:57m:39s]----
*/
...
protected:
    ...
    template<typename _Callback>
    constexpr bool _M_for_each(int p, int l, int r, int x, int y, const _Callback& __func)
    {
        if (l == r)
        {
...
int main(void)
{
    ...
    while (_--)
    {
        ...
        for (auto [it, i] = std::tuple(map.begin(), 1); it != map.end(); ++it, ++i)
        {
            it->second = i;
        }

        const int64 inf = std::numeric_limits<int64>::min() / 2;

        std::vector<int64> dp(n + 1, inf);
        dp[0] = 0;

        std::array<SegmentTree<int64, decltype(std::ranges::max)>, 3> segs;

        for (auto& seg : segs)
        {
            seg.resize(map.size() + 1);
            seg.set(inf);
            seg.set(map[0], 0);
        }
        ...
        for (int64 i = 1; i <= n; ++i)
        {
            dp[i] = std::max(dp[i], segs[0].reduce(0, map[v[i]] - 1) + i);
            dp[i] = std::max(dp[i], segs[1].at(map[v[i]]));
            dp[i] = std::max(dp[i], segs[2].reduce(map[v[i]] + 1, map.size() + 1) - i);

            segs[0].set(map[v[i]], dp[i] - i, std::ranges::max);
            segs[1].set(map[v[i]], dp[i],     std::ranges::max);
            segs[2].set(map[v[i]], dp[i] + i, std::ranges::max);
        }
