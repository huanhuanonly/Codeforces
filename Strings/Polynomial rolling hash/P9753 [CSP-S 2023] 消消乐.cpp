 ...
 * CSP-S 2023 自测【官方数据】
 *
 * => P9753 [CSP-S 2023] 消消乐 ( @c 提高+/省选− )
 *
 * -> https://www.luogu.com.cn/problem/P9753
 *
 * Submissions ~> https://www.luogu.com.cn/record/278727954 By huanhuanonly
 *
 * @b Polynomial-rolling-hash
 *
 * ----May 21, 2026 [15h:42m:45s]----
*/
...
{
using hash_result_type = uint128;

constexpr hash_result_type hash_base = std::array{449ULL, 100003ULL, 1000003ULL, 1000000007ULL, 911382323ULL}[4];
constexpr hash_result_type hash_mod  = std::array{998244353ULL, 4294967291ULL, 1ULL << 61 - 1}[2];
...
int main(void)
{
    ...
    std::map<hash_result_type, uint64> map;
    map[0] = 1;

    for (char c : s)
    {
        if (not stack.empty() and stack.back() == Hash<char>{}(c) % hash_mod)
        {
            stack.pop_back();
        }
        else
