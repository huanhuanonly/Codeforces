 ...
 *
 * => F. It Just Keeps Going Sideways ( @c 1700 )
 *
 * -> https://codeforces.com/contest/2227/problem/F
 *
 * Submissions ~> https://codeforces.com/contest/2227/submission/373174383 By huanhuanonly
 *
 * @b Segment-tree
 *
 * ----May 01, 2026 [03h:45m:16s]----
*/
...
int main(void)
{
    ...
    while (_--)
    {
        ...
        LazySegmentTree<int64> seg(n + 1);
        seg.fill(n - 1);

        for (int64 i = n - 1; i >= 0; --i)
        {
            ans += seg.reduce(1, v[i]) - v[i] * i;
            seg.range_update(1, v[i], -1);
        }
        ...
        for (int64 i = 0; i < n; ++i)
        {
            max = std::max(max, seg.reduce(v[i], n));
            seg.range_update(v[i], v[i], 1);
        }
