 ...
 * 
 * => G. Query Jungle ( @c 2900 )
 * 
 * -> https://codeforces.com/contest/2152/problem/G
 * 
 * Submissions ~> https://codeforces.com/contest/2152/submission/343171118 By huanhuanonly
 * 
 * @b Euler-order and @b Lazy-segment-tree ( @a to-find-the-length-of-the-specified-longest-subsequence )
 * 
 * ----October 12, 2025 [00h:55m:49s]----
*/
...
constexpr void chmax(auto& a, const auto& b)
{
    if (a < b)
    {
        a = b;
...
int main(void)
{
    ...
    while (_--)
    {
        ...
        auto dfs = [&, num = -1](auto& self, int u, int fa) mutable -> void
        {
            in[u] = ++num;
        ...
        struct node
        {
            ...
            {
                return std::max(len[0], len[1]) / 2;
            }

            struct merge
            {
                static int r(int x) noexcept
                {
                    if (x < 2)
                    {
                        return not x;
                ...
                node operator()(const node& fv, const node& sv) const noexcept
                {
                    node nd{};

                    for (int i = 0; i < nd.len.size(); ++i)
                    {
                        if (fv.len[i] & 1)
                        {
                            nd.len[i] = fv.len[i] + sv.len[r(i)];

                            if (fv.len[i])
                            {
                                chmax(nd.len[i], fv.len[i] - 1 + sv.len[i]);
                            }
                        }
                        else
                        {
                            nd.len[i] = fv.len[i] + sv.len[i];

                            if (fv.len[i])
                            {
                                chmax(nd.len[i], fv.len[i] - 1 + sv.len[r(i)]);
                            }
                        }
                    }
        ...
        LazySegmentTree<node, node::merge, int, decltype([](const auto& that, const auto& value) -> void {
            if (value)
            {
                that.lazy = not that.lazy;

                std::swap(that.value.len[0], that.value.len[2]);
                std::swap(that.value.len[1], that.value.len[3]);
            }
        })> seg(seq.size());
        ...
        while (q--)
        {
            ...
            seg.range_update(in[u], out[u], true);
