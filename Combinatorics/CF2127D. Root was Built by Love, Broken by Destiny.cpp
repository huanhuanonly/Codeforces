 ...
 * 
 * => D. Root was Built by Love, Broken by Destiny ( @c 1800 )
 * 
 * -> https://codeforces.com/contest/2127/problem/D
 * 
 * Submissions ~> https://codeforces.com/contest/2127/submission/334572896 By huanhuanonly
 * 
 * @b Combinatorics and @b Trees
 * 
 * ----August 20, 2025 [01h:28m:54s]----
*/
...
int main(void)
{
    ...
    std::vector<Modular<uint64, mod>> factorial(2e5 + 1, 1);

    for (uint64 i = 2; i < factorial.size(); ++i)
    {
    ...
    while (_--)
    {
        ...
        auto check = [&, vis = std::vector<bool>(n + 1)](auto& self, int u, int fa) mutable -> bool
        {
            if (vis[u])
            {
                return false;
        ...
        if (m == n - 1 and check(check, 1, 0))
        {
            Modular<uint64, mod> ans = 2;
