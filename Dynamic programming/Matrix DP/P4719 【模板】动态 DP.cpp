/**
 * => P4719 【模板】动态 DP ( @c 省选/NOI- )
 *
 * -> https://www.luogu.com.cn/problem/P4719
 *
 * Submissions ~> https://www.luogu.com.cn/record/276950614 By huanhuanonly
 *
...
int main(void)
{
    ...
    auto decompose = [&, id = 0](auto& self, int u, int head) mutable -> void
    {
        tree[u].head = head;
        tree[u].dfn = ++id;
        tail[head] = u;
    ...
    SegmentTree<matrix22, decltype([](const matrix22& lhs, const matrix22& rhs) {
    ...
    auto dfs = [&](const auto& self, int u) -> void
    {
        ...
        seg.set(tree[u].dfn, matrix22{{g[u][0], g[u][1]}, {g[u][0], -inf}});
    };
    ...
    {
        const auto& m = seg.reduce(tree[head].dfn, tree[tail[head]].dfn);
