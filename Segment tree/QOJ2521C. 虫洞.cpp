 ...
 * 
 * Submissions ~> https://qoj.ac/submission/2167512 By huanhuanonly
 * 
 * @b Segment-tree
 * 
 * ----March 26, 2026 [23h:41m:29s]----
*/
    ...
    while (_--)
    {
        ...
        for (int l = 0, r = -1; ; )
        {
            if (seg.reduce() <= k)
            {
                ...
                seg.range_update(v[r].first, v[r].second, 1);
            }
            else
            {
                seg.range_update(v[l].first, v[l].second, -1);
                ++l;
            }
        }
