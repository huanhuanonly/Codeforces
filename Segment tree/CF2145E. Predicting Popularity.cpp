 ...
 * 
 * => E. Predicting Popularity ( @c 2100 )
 * 
 * -> https://codeforces.com/contest/2145/problem/E
 * 
 * Submissions ~> https://codeforces.com/contest/2145/submission/344046257 By huanhuanonly
 * 
 * @b Segment-tree and @b Binary-search
 * 
 * ----October 16, 2025 [22h:53m:05s]----
*/
...
int main(void)
{
    ...
    auto $ = [ac, dr](int a, int d) -> int
    {
        return std::max(a - ac, 0) + std::max(d - dr, 0);
    };

    constexpr int maxv = 1e6;

    LazySegmentTree<int, decltype(std::ranges::min), int, MakeLazyFunc::Add> seg(maxv + 1);

    seg.for_each([i = 0](int& value) mutable -> bool
    {
        value = i--;
        return true;
    });
    ...
    for (int i = 1; i <= n; ++i)
    {
        seg.range_update($(a[i], d[i]) + 1, maxv, 1);
    }
    ...
    while (q--)
    {
        ...
        {
            return seg.reduce(0, mid) >= 0;
        };

        int l = 0, r = maxv + 1, mid;
