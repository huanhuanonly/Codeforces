 ...
 * 
 * => D. Antiamuny Wants to Learn Swap ( @c 1900 )
 * 
 * -> https://codeforces.com/contest/2139/problem/D
 * 
 * Submissions ~> https://codeforces.com/contest/2139/submission/337758643 By huanhuanonly
 * 
 * @b Segment-tree
 * 
 * ----September 09, 2025 [21h:13m:22s]----
*/
...
int main(void)
{
    ...
    while (_--)
    {
        ...
        std::vector<int> prev(n + 1), next(n + 1, n + 1);
        ...
        // build `next`
        {
            std::set<std::pair<int, int>, std::ranges::greater> st;
        ...
        std::vector<int> min(n + 1, n + 1);

        for (int i = 1; i <= n; ++i)
        {
            min[prev[i]] = std::min(min[prev[i]], next[i]);
        }

        SegmentTree seg(min, static_cast<const int& (*)(const int&, const int&)>(&std::min));
