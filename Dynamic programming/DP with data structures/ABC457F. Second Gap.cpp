 ...
 *
 * Submissions ~> https://atcoder.jp/contests/abc457/submissions/75776717 By huanhuanonly
 *
 * @b DP with @b Segment-tree
 *
 * ----May 14, 2026 [10h:03m:39s]----
*/
...
int main(void)
{
    ...
    LazySegmentTree<uint64, std::plus<>, uint64, decltype([](const auto& that, const auto& value) -> void {
        if (that.marked)
        {
            (that.lazy *= value) %= mod;
        }
    ...
    for (int i = n - 3; i >= 0; --i)
    {
        ...
        const int j = i + d[i];
        const uint64 t = dp.at(j) % mod;

        if (n - i - 2 >= 0 and d[i] == d[i + 1])
        {
            dp.range_update(i + 1, n - 1, n - i - 2);
        }
        else
        {
            dp.range_update(0, n - 1, 0);
        }

        dp.set(i, (dp.at(i) + t) % mod);
        dp.set(j, (dp.at(j) + t) % mod);
    }

    std::cout << dp.reduce() % mod << '\n';
