 ...
 *
 * Submissions ~> https://codeforces.com/contest/2227/submission/373520525 By huanhuanonly
 *
 * @b Prefix-sum and @b Segment-tree
 *
 * ----May 04, 2026 [23h:20m:16s]----
*/
...
int main(void)
{
    ...
    while (_--)
    {
        ...
        const int maxv = s.size() - 1;

        std::array segs{SegmentTree<int>(maxv + 1), SegmentTree<int>(maxv + 1)};
        ...
        for (int i = 0; i <= n; ++i)
        {
            if (i & 1)
            {
                if (v[i] + 1 <= maxv)
                {
                    ans += segs[i & 1 ^ 1].reduce(v[i] + 1, maxv);
                }
            }
            else
            {
                if (0 <= v[i] - 1)
                {
                    ans += segs[i & 1 ^ 1].reduce(0, v[i] - 1);
                }
            }

            segs[i & 1].set(v[i], segs[i & 1].at(v[i]) + 1);
        }
