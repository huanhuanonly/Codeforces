 ...
 * 
 * Submissions ~> https://atcoder.jp/contests/dp/submissions/75003716 By huanhuanonly
 * 
 * @b DP with @b Segment-tree
 * 
 * ----April 17, 2026 [01h:08m:35s]----
*/
...
int main(void)
{
    ...
    std::vector<uint64> dp(n);

    SegmentTree<uint64, decltype(std::ranges::max)> seg(n + 1);

    for (int i = 0; i < n; ++i)
    {
        dp[i] = seg.reduce(0, h[i] - 1) + v[i];
        seg.set(h[i], dp[i]);
    }
