 ...
 * => E. 乐乐的杏花
 *
 * -> https://codeforces.com/gym/106534/problem/E
 *
 * Submissions ~> https://codeforces.com/gym/106534/submission/375379428 By huanhuanonly
 *
 * @b Dfs-order and @b Lazy-segment-tree
 *
 * ----May 21, 2026 [01h:23m:27s]----
*/
...
int main(void)
{
    ...
    LazySegmentTree<
        node,
        decltype([](const node& lhs, const node& rhs) -> node
        {
            if (not lhs.bloomed)
            {
                return rhs;
            }
            else if (not rhs.bloomed)
            {
                return lhs;
            }
            else
            {
                return {std::min(lhs.dist, rhs.dist), true};
            }
        }),
        int,
        decltype([](const auto& that, const auto& value) -> void
        {
            that.value.dist += value;
            that.lazy += value;
        })
    > seg(n);
    ...
    auto dfs = [&, idx = -1](auto& self, int u) mutable -> void
    {
        dfn[u] = ++idx;
        sizes[u] = 1;

        seg.set(dfn[u], node{depth[u], false});
    ...
    while (m--)
    {
        ...
        if (opt == 1)
        {
            const auto value = seg.at(dfn[x]);
            seg.set(dfn[x], node{value.dist, not value.bloomed});
        }
        else
        {
            if (x == fa[cpos])
            {
                seg.range_update(0, seg.size() - 1, -1);
                seg.range_update(dfn[cpos], dfn[cpos] + sizes[cpos] - 1, +2);
            }
            else
            {
                seg.range_update(0, seg.size() - 1, +1);
                seg.range_update(dfn[x], dfn[x] + sizes[x] - 1, -2);
            }

            cpos = x;
        }

        if (const auto result = seg.reduce(); result.bloomed)
        {
